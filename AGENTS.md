...
Moving or renaming a page changes its URL. Add a rule to `public/_redirects` whenever you do, and retarget any existing rule that pointed at the old path — the file is ordered most-specific-first and every generation of the site's URLs is still mapped there.
...
Within each autogenerated section, subdirectories become sub-sections and their `_meta.yml` sets the label and order. A directory's `index.md` becomes a link inside its own section (this is how the "Overview" entries work).

To add a new top-level section, add an entry to the `sidebar` array in `astro.config.mjs`, and a matching Keystatic collection in `keystatic.config.tsx`.
